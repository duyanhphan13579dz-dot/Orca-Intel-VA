        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {news.map((n) => (
              ...
              <div className="relative h-44 w-full overflow-hidden">
                ...
                <img 
                  src={n.image || "/news-placeholder.jpg"} 
                  alt={n.title} 
                  className="h-full w-full object-cover transition duration-500 group-hover:scale-110 bg-navy-deep" 
                  loading="lazy"
                />
                <div className="absolute top-2 right-2">
                  <Badge tone={n.impact === "cao" ? "danger" : n.impact === "trung bình" ? "gold" : "neutral"}>
                    {n.impact}
                  </Badge>
                </div>
              </div>
              <div className="p-4">
                <div className="mb-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] text-muted">
                  <span className="font-bold text-gold uppercase">{n.source}</span>
                  <span>•</span>
                  <span>{new Date(n.publishedAt).toLocaleDateString("vi-VN")} {new Date(n.publishedAt).toLocaleTimeString("vi-VN")}</span>
                </div>
                <h3 className="line-clamp-2 text-sm font-bold leading-snug group-hover:text-gold">{n.title}</h3>
                <p className="mt-2 line-clamp-2 text-[11px] leading-relaxed text-muted/80">{n.summary}</p>
                <div className="mt-3 flex items-center justify-between border-t border-white/5 pt-2 text-[9px] text-muted italic">
                  <span>Đồng bộ: {new Date(n.syncedAt).toLocaleTimeString("vi-VN")}</span>
                  <span className="text-gold/50">Orca News Engine</span>
                </div>
              </div>
            </Link>
          ))}
