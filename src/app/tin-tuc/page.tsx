      ...
      {featured && (
        <Link href={`/tin-tuc/${featured.slug}`} className="glass group mb-6 grid overflow-hidden rounded-xl md:grid-cols-2">
          ...
          <div className="p-6">
            ...
            <div className="mt-3 text-xs text-muted"><span className="text-gold">{featured.source}</span> • {new Date(featured.publishedAt).toLocaleString("vi-VN")}</div>
          </div>
        </Link>
      )}
        ...
        {rest.map((n) => (
          <Link key={n.slug} href={`/tin-tuc/${n.slug}`} className="glass group overflow-hidden rounded-xl transition hover:border-gold/40">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 text-[11px] text-muted"><span className="text-gold">{n.source}</span> • {new Date(n.publishedAt).toLocaleTimeString("vi-VN")}</div>
            </div>
          </Link>
        ))}
      </div>
